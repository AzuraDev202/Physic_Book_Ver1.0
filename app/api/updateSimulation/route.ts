    ...
    try {
        ...
        for (const lesson of chapter.lessons) {
            ...
            if (simulationSlides[lessonId]) {
                // Xóa tất cả các slide simulation cũ có cùng simulationType
                lesson.slides = lesson.slides.filter((slide: any) => 
                    !(slide.type === 'simulation' && slide.simulationType === simulationSlides[lessonId].simulationType)
                )
                
                // Thêm slide simulation mới
                lesson.slides.push(simulationSlides[lessonId])
                console.log(`Updated simulation slide for lesson ${lessonId}`)
            }
        }
